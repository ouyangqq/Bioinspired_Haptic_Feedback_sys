...
class tactile_receptors():
    ...
    def set_population(self,punit_buf,grid_buf,simTime=1,sample_rate=1000,Density=150,youngs=50*1e3,roi=[]):  
        ...
        '''
        SC=np.zeros([self.Nr,self.Nc])
        RI=SC[0:self.Nr:self.Nds,0:self.Nc:self.Nds]
        self.Nrc=RI.shape[1]
        self.Nrr=RI.shape[0]
        '''
...
def step_wave(X,Tstart,Tend,rate,rate1,intentation):
    ...
    return state4
def step_wave_any(X,Tstart,Tend,rate,rate1,peak):
    state1=rate*(X-Tstart)*(X>Tstart)
    state2=state1+(peak-state1)*(state1>peak)
    state3=state2+(peak+rate1*(X-Tend)-state2)*(X>Tend)
    state4=state3+(0-state3)*(state3<0)
    return state4
